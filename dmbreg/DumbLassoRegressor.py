...
class DumbLassoRegressor:
    def __init__(self):
        self.beta = None
        self.loss_history = None
        self.epochs = None
    
    def __loss(self, X, Y, lambd):
        K = len(Y)
        residual = Y - X @ self.beta
        MSE = (residual @ residual)/(2*K)
        abs_beta_sum = np.abs(self.beta).sum()
        return MSE + lambd * abs_beta_sum
    ...
    def fit(self, X : np.ndarray,
        ...
        best_metric = self.__loss(X, Y, lambd)
        patience_calc = 0
        K = len(Y)
        for epoch in range(epochs):
            for j in range(len(self.beta)):
                ...
                z = (rj @ X[:,j])/K
                self.beta[j] = self.__soft_treeshold(z, lambd)/norm
        
            loss = self.__loss(X, Y, lambd)
